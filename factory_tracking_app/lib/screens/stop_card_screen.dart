...
class StopCardScreen extends StatefulWidget {
  const StopCardScreen({super.key});

  @override
  State<StopCardScreen> createState() => _StopCardScreenState();
}

class _StopCardScreenState extends State<StopCardScreen> {
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 10),
            Text(
              '${_labelController.text.length}/60',
              style: TextStyle(
                fontSize: 12,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: _isSubmitting || _labelController.text.trim().isEmpty
                    ? null
                    : _submitStopCard,
    ...
    try {
       ...
       if (compressedImage != null) {
         ...
         if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
             const SnackBar(
               content: Text('Image captured and compressed'),
    ...
    try {
       ...
       if (compressedImages.isNotEmpty) {
         ...
         if (mounted) {
           ScaffoldMessenger.of(context).showSnackBar(
             SnackBar(
               content: Text('${compressedImages.length} images compressed and added'),
               backgroundColor: Colors.green,
             ),
           );
         }
  ...
  Future<void> _submitStopCard() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(
            builder: (context) => const StopCardSuccessScreen(),
          ),
        );
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error submitting stop card: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
...
class StopCardSuccessScreen extends StatelessWidget {
  const StopCardSuccessScreen({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text(
          'Your Stop Card Details Is sent',
          style: TextStyle(
            fontSize: 20,
            fontWeight: FontWeight.bold,
            color: Colors.black87,
